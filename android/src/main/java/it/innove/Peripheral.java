...
public class Peripheral extends BluetoothGattCallback {
	...
	public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
		...
		if (newState == BluetoothGatt.STATE_CONNECTED) {
			...
			sendConnectionEvent(device, "BleManagerDisconnectPeripheral");
            List<Callback> callbacks = Arrays.asList(writeCallback, retrieveServicesCallback, readRSSICallback, readCallback, registerNotifyCallback, requestConnectionPriorityCallback);
            ...
            requestConnectionPriorityCallback = null;
		}

	}

    public void requestConnectionPriority(int priority, Callback callback){
        if (!isConnected()) {
        ...
        if (Build.VERSION.SDK_INT >= LOLLIPOP) {
            requestConnectionPriorityCallback = callback;
            gatt.requestConnectionPriority(priority);
        } else {
            callback.invoke("The requestConnectionPriority require at least 21 API level", null);
        }
    }
